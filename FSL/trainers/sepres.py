...
from .clip_text_sep import clip
from .clip_text_sep.simple_tokenizer import SimpleTokenizer as _Tokenizer
import tqdm
...
def load_clip_to_cpu(cfg):
    ...
    if cfg.MODEL.FORGET_PATH:
        print("LOADING FROM FORGET PATH", cfg.MODEL.FORGET_PATH)
        if 'model_dict' not in torch.load(cfg.MODEL.FORGET_PATH):
            model.load_state_dict(torch.load(cfg.MODEL.FORGET_PATH))
        else:
            model.load_state_dict(torch.load(cfg.MODEL.FORGET_PATH)['model_dict'], strict=False)
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model,device):
        ...
        # TaskRes Part
        text_encoder = TextEncoderRes(clip_model) 
        ...
        self.image_feature_residuals = nn.Parameter(torch.zeros_like(base_text_features[0:1]))
    ...
    def forward(self):
        ...
        return prompt, self.prompt_projections, self.ctx_visual, self.visual_prompts, self.text_feature_residuals
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None,feat_flag=False,proto=None):
        ...
        prompts,prompt_projections,visual_ctx, visual_prompts, text_feature_residuals = self.prompt_learner()
        with torch.no_grad():
        ...
        text_features = text_features + text_features_old + self.alpha * text_feature_residuals 
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
...
class SEPRES(TrainerX):
    ...
    def build_model(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            if name_to_update not in name and "VPT" not in name and "text_feature_residuals" not in name:# and "VPT_fc" in name:
    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            # set strict=False
            print(self._models[name].load_state_dict(state_dict, strict=False))