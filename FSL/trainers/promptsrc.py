...
from clip_promptsrc import clip
from clip_promptsrc.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
def load_clip_to_cpu(cfg, zero_shot_model=False):
    ...
    if not zero_shot_model:
        ...
        #     # init_proj = model.text_projection.clone()
        #     # print(init_proj)
        #     model.load_state_dict(torch.load(cfg.MODEL.FORGET_PATH)['model_dict'], strict=False)
        #     # final_proj =  model.text_projection.clone()
        #     # print("SAME", torch.allclose(init_proj, final_proj))
        #     # print(final_proj)
        if cfg.MODEL.FORGET_PATH:
            print("LOADING FROM FORGET PATH", cfg.MODEL.FORGET_PATH)
            if 'model_dict' not in torch.load(cfg.MODEL.FORGET_PATH):
                print("LOADING WITH FIRST IF")
                model.load_state_dict(torch.load(cfg.MODEL.FORGET_PATH), strict=False)
            else:
                print("LOADING WITH SECOND IF")
                model.load_state_dict(torch.load(cfg.MODEL.FORGET_PATH)['model_dict'], strict=False)
