...
class TrainerBase:
    ...
    def model_backward_and_update(self, loss, names=None):
        # AK: it does it for models' names that are in 
        # `names` variable
...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        # if self.cfg.TRAIN.SAVE_PROBS: 
        # print("output", output.shape, output.argmax(-1))
        # print("label", label)
        ...
        save_results = {'all_outputs' : all_outputs, 'all_labels' : all_labels}
        save_path = f"{self.cfg.OUTPUT_DIR}/{self.cfg.DATASET.NAME}_shots_{self.cfg.DATASET.NUM_SHOTS}_seed_{self.cfg.SEED}.pkl"
    ...
    def get_current_lr(self, names=None):
        names = self.get_model_names(names)
        name = names[0]
        all_lrs = [pg["lr"] for pg in self._optims[name].param_groups]
        return all_lrs
